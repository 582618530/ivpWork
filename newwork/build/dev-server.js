...
apiRoutes.get('/channel', function(req, res) {
  let channel = ''
  let url = encodeURI('http://api.jisuapi.com/news/channel?appkey=c27e5d9d647eeb1c')
...
apiRoutes.get('/channel/:item', function(req, res) {
  ...
  function findChannel(item) {
    return new Promise((resolve, reject) => {
      let findResult = ''
      let url = encodeURI('http://api.jisuapi.com/news/get?channel='+item+'&start=0&num=30&appkey=c27e5d9d647eeb1c')/* key码 */
      http.get(url, response => {
        response.on('data', data => {
          findResult += data
        })
...
apiRoutes.get('/search/:keywords', function(req, res) {
  ...
  function search(keywords) {
    return new Promise((resolve, reject) => {
      let searchResult = ''
      let url = encodeURI('http://api.jisuapi.com/news/search?keyword='+keywords+'&appkey=c27e5d9d647eeb1c')
      http.get(url, response => {
        response.on('data', data => {
          searchResult += data
        })
